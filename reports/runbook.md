...
|---|---|---|---|---|
| 1 | Xác nhận outage | `.venv\Scripts\python.exe chaos/kill_region.py status` | `a.alive=false` 3 lần liên tiếp | on-call |
| 2 | Mở incident + bấm giờ RTO | `.venv\Scripts\python.exe dr/runbook.py --primary a --target b --backend fs --auto` | ts ghi vào `reports/runbook-run.jsonl` | on-call |
...
| 7 | Đo RTO + postmortem | `.venv\Scripts\python.exe tools/measure_rto.py --loadgen reports/drill-2-withdr.jsonl --target-rto 300` | `rto_verdict` = PASS hoặc NO_RECOVERY | on-call |
...
- **Lệnh rollback:**
  ...
  ```
- **Lưu ý:** Không chạy full-auto failover mà không có circuit breaker - sẽ gây flap giữa 2 region liên tục.
